...
import { generateSudoku, solveSudoku, getNextHint, cloneSudoku } from '@sudoku/sudoku';
...
function createUserGrid() {
	...
	return {
		...
		undo() {
			const current = get(userGrid);
			const prevState = history.undo(current);
			if (prevState) {
				userGrid.set(prevState);
			}
		},

		redo() {
			const current = get(userGrid);
			const nextState = history.redo(current);
			if (nextState) {
...
export const needsExplore = derived(userGrid, $userGrid => {
	return getNextHint($userGrid) === null;
});
